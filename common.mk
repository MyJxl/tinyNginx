...
#定义存放obj文件的目录，目录统一到一个位置才方便后续链接，不然整到各个子目录去，不好链接
...
#处理后形如 ~/nginx/app/link_obj/ngx_signal2.o ~/nginx/app/link_obj/ngx_signal.o
# := 在解析阶段直接赋值常量字符串【立即展开】，而 = 在运行阶段，实际使用变量时再进行求值【延迟展开】
# ~/nginx/app/link_obj/nginx.o   ~/nginx/app/link_obj/ngx_conf.o 
...
#上行处理后，.d文件中内容应该就如：~/nginx/app/link_obj/nginx.o: nginx.c ngx_func.h ../signal/ngx_signal.h
