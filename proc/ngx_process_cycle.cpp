...
{    
    ...
    //设置,此时不打算接受的信号: 阻塞期间, 你发过来的上述信号, 多个会被合并为一个, 
    // 暂存着,等你放开信号屏蔽后才能收到这些信号。。。
    ...
    ngx_start_worker_processes(workprocess);  //这里要创建worker子进程 // 子进程在内部进行死循环
    ...
    //    usleep(100000);
        ngx_log_error_core(0,0,"Master Process, pid is %P",ngx_pid);
...
static int ngx_spawn_process(int inum,const char *pprocname)
{
    ...
    {  
    case -1: //产生子进程失败
        ngx_log_error_core(NGX_LOG_ALERT,errno,"ngx_spawn_process() fork() worker process num=%d, procname=\"%s\" failed!",inum,pprocname);
        return -1;
